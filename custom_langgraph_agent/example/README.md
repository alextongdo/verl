# MathExpression: Modified LangGraph Agent Example
...
- name: math_expression
  _target_: custom_langgraph_agent.example.math_expression.MathExpressionReactAgentLoop
```

### Prepare dataset
Now, let's prepare two small datasets for training and evaluation:
```bash
python custom_langgraph_agent/example/create_dataset.py
```
...
```bash
bash custom_langgraph_agent/example/run_qwen2.5_3b.sh 2>&1 | tee train.log
```

To submit on a SLURM cluster (the script contains SBATCH headers):
```bash
sbatch custom_langgraph_agent/example/run_qwen2.5_3b.sh
```
...
```bash
GPUS_PER_NODE=4 NNODES=2 bash custom_langgraph_agent/example/run_qwen2.5_3b.sh
```
